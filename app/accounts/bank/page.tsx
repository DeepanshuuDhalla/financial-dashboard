...
import { Plus, Edit, Trash2, X, Save, Wallet, Banknote } from "lucide-react";
...
const BankAccountsPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.type || !form.currency || form.balance === undefined) return;
    if (editAccount) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"><Plus size={18}/>Add Account</button>
      </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded shadow-lg p-6 w-full max-w-md relative">
            ...
            <h2 className="text-lg font-semibold mb-4">{editAccount ? 'Edit' : 'Add'} Account</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select value={form.type || 'checking'} onChange={e => handleChange('type', e.target.value)} className="w-full border px-3 py-2 rounded">
