...
const Investments: React.FC<{ userId: string }> = ({ userId }) => {
  const { data, addAccount, updateAccount, deleteAccount, reload } = useFinancialData(userId);
  const [accounts, setAccounts] = useState<Account[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [editAccount, setEditAccount] = useState<Account | null>(null);
  const [form, setForm] = useState<Partial<Account>>({ type: "investment", currency: "USD", isActive: true });
  ...
  useEffect(() => {
    if (data && data.accounts) {
      ...
      if (investmentAccounts.length === 0) {
        setAccounts(MOCK_ACCOUNTS.filter((a: Account) => a.type === "investment"));
  ...
  const investmentCount = accounts.length;

  // Pie chart data
  const allocation = accounts.map((a, i) => ({ name: a.name, value: a.balance, color: COLORS[i % COLORS.length] }));

  const openAdd = () => {
    setEditAccount(null);
    setForm({ type: "investment", currency: "USD", isActive: true });
    setModalOpen(true);
    setOpError(null);
  };
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card gradient>
          <CardContent>
            <div className="flex items-center gap-3 mb-2">
              <span className="w-5 h-5 bg-blue-100 rounded-full flex items-center justify-center"><span className="text-blue-600 font-bold">$</span></span>
              <span className="text-xs font-medium text-gray-700">Total Investment</span>
            </div>
        ...
        <Card>
          <CardContent>
            ...
            <div className="text-2xl font-bold text-gray-900">{investmentCount}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardContent>
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={openAdd} size="default" variant="default" className="flex items-center gap-2"><Plus size={18}/>Add Investment</Button>
          </div>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search investments..." className="mb-4 px-3 py-2 border rounded w-full" />
          <div className="overflow-x-auto rounded-xl">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {filtered.length === 0 && <tr><td colSpan={6} className="p-4 text-center text-gray-500">No investments found.</td></tr>}
              </tbody>
            </table>
          </div>
        </CardContent>
      ...
      <FullscreenModal isOpen={modalOpen} onClose={closeModal}>
        <h2 className="text-2xl font-bold mb-6 text-center text-blue-900 tracking-wide drop-shadow-lg">{editAccount ? 'Edit' : 'Add'} Investment</h2>
