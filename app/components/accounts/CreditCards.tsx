...
import { Plus, Edit, Trash2, Save, CreditCard } from "lucide-react";
...
const CreditCards: React.FC<{ userId: string }> = ({ userId }) => {
  ...
  useEffect(() => {
    if (data && data.accounts) {
      const creditAccounts = data.accounts.filter((a: Account) => a.type === "credit");
      if (creditAccounts.length === 0) {
        setAccounts(MOCK_ACCOUNTS.filter((a: Account) => a.type === "credit"));
  ...
  const utilization = totalLimit ? ((Math.abs(totalBalance) / totalLimit) * 100).toFixed(1) : "0";
  const cardCount = accounts.length;

  // Mock trend data
  const balanceTrend = [
    { month: "Jan", balance: totalBalance * 0.7 },
    { month: "Feb", balance: totalBalance * 0.8 },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setOpError(null);
    if (!form.name || !form.currency || form.balance === undefined || !form.bank) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent>
            ...
            <div className="text-2xl font-bold text-gray-900">{cardCount}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-xs font-medium text-gray-700">Total Limit</span>
            </div>
            <div className="text-2xl font-bold text-gray-900">${totalLimit.toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={balanceTrend}>
                ...
                <YAxis stroke="#9ca3af" />
                <Tooltip />
                <Line type="monotone" dataKey="balance" stroke="#ef4444" strokeWidth={3} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
      ...
      <Card>
        <CardContent>
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={openAdd} size="default" variant="default" className="flex items-center gap-2"><Plus size={18}/>Add Card</Button>
          </div>
      ...
      <FullscreenModal isOpen={modalOpen} onClose={closeModal}>
        <h2 className="text-2xl font-bold mb-6 text-center text-blue-900 tracking-wide drop-shadow-lg">{editAccount ? 'Edit' : 'Add'} Credit Card</h2>
        <form onSubmit={handleSubmit} className="space-y-6 p-4 sm:p-8 w-full max-w-2xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block mb-2 text-sm font-semibold text-gray-700">Credit Limit</label>
              <input type="number" value={form.creditLimit ?? ''} onChange={e => handleChange('creditLimit', parseFloat(e.target.value))} className="w-full border border-blue-200 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white/90" />
            </div>
